...
bool pawn_history_piece(PieceType pt) {
  return pt == PAWN || pt == SOLDIER;
}

bool minor_correction_piece(PieceType pt) {
  return pt == KNIGHT || pt == HORSE || pt == CANNON;
}
...
/*
   ...
   5) Halfmove clock. This is the number of halfmoves since the last pawn advance
      ...
      configured n-move rule, which is 60 moves for Xiangqi.
...
void Position::set_state(StateInfo* si) const {
  ...
  si->key = si->materialKey = si->minorPieceKey = 0;
  si->pawnKey = Zobrist::noPawns;
  si->nonPawnKey[WHITE] = si->nonPawnKey[BLACK] = 0;
  ...
  for (Bitboard b = pieces(); b; )
  {
      ...
      else if (pawn_history_piece(type_of(pc)))
          si->pawnKey ^= Zobrist::psq[pc][s];

      else if (type_of(pc) != KING)
      {
          si->nonPawnMaterial[color_of(pc)] += PieceValue[MG][pc];
          si->nonPawnKey[color_of(pc)] ^= Zobrist::psq[pc][s];

          if (minor_correction_piece(type_of(pc)))
              si->minorPieceKey ^= Zobrist::psq[pc][s];
      }
  }
...
string Position::fen(bool sfen, bool showPromoted, int countStarted, std::string holdings, Bitboard fogArea) const {
  ...
  // Counting ply or n-move rule counter
  if (st->countingLimit)
...
bool Position::legal(Move m) const {
  ...
  // Makpong rule
  if (makpong() && checkers() && type_of(moved_piece(m)) == KING && (checkers() ^ to))
      return false;
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  if (captured)
  {
      ...
      // If the captured piece is a pawn/soldier, update pawn hash key, otherwise
      // update non-pawn material.
      if (pawn_history_piece(type_of(captured)))
      ...
      else
      {
          ...
          if (minor_correction_piece(type_of(captured)))
  ...
  // If the moving piece is a pawn/soldier do some special extra work
  if (pawn_history_piece(type_of(pc)))
  {
  ...
  if (type_of(m) == NORMAL && !pawn_history_piece(type_of(pc)) && type_of(pc) != KING)
  {
...
/// n-move rule, by repetition, or a variant rule that allows a player to claim a game result.
...
bool Position::is_optional_game_end(Value& result, int ply, int countStarted) const {
  ...
  if (n_move_rule() && st->rule50 > (2 * n_move_rule() - 1) && (!checkers() || MoveList<LEGAL>(*this).size()))
  {
      ...
      if (var->chasingRule == AXF_CHASING && st->pliesFromNull >= 20)
      {
          ...
          offset = 2 * std::max(std::max(checkThem, checkUs) - 10, 0) + 20 * (CurrentProtocol == UCCI);
      }
...
void Position::flip() {
  ...
  set(variant(), f, false, st, this_thread());
