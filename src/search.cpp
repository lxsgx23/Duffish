...
namespace {
  ...
  // Futility margin
  Value futility_margin(Depth d, bool improving, int aggressiveness) {
    int aggressFactor = 200 - aggressiveness;
    return Value(214 * aggressFactor / 100 * (d - improving));
  }
  ...
  Value value_draw(Thread* thisThread, Color us) {
    Value contempt = (us == thisThread->rootColor) ? Value(thisThread->cachedDrawValue) : VALUE_ZERO;

    int drawMatBias = thisThread->cachedDrawMatBias;
    if (drawMatBias != 0 && us == thisThread->rootColor)
    {
        Position& pos = thisThread->rootPos;
        int matDiff = pos.non_pawn_material(WHITE) - pos.non_pawn_material(BLACK);
        int matImbalance = (us == WHITE) ? matDiff : -matDiff;
        contempt += Value(drawMatBias * matImbalance / 500);
    }
...
void Thread::search() {
  ...
  cachedIsChessVariant = (Options["UCI_Variant"] == "chess");
  ...
  ss->materialDiff = rootPos.non_pawn_material(WHITE) - rootPos.non_pawn_material(BLACK);
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              int tr = dynamicComp * prev / (abs(prev) + 147);
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!rootNode && TB::Cardinality)
    {
        ...
        if (    piecesCount <= TB::Cardinality
            ...
            &&  pos.rule50_count() == 0
            &&  thisThread->cachedIsChessVariant
    ...
    // Patricia-style sacrifice detection bonus (Patricia aggressive mechanism)
    // Detects sacrifice patterns in the search tree by examining material differences
    // at different plies. When a sacrifice pattern is found (material going down
    // then up alternately), gives a bonus to incentivize finding sacrificial continuations.
    {
        int sacDetect = (us == thisThread->rootColor) ? thisThread->cachedSacDetect : 0;
        if (sacDetect > 0 && !ss->inCheck && ss->ply >= 4 && eval != VALUE_NONE)
        {
            ...
            {
                int matDiff = pos.non_pawn_material(WHITE) - pos.non_pawn_material(BLACK);
                int rootMatDiff = (ss-4)->materialDiff;
                int matChange = matDiff - rootMatDiff;
                int matImbalance = (us == WHITE) ? -matChange : matChange;
                if (matImbalance > 0)
                {
                    int bonus = sacDetect * matImbalance / 200;
                    if (abs(eval) > 500)
                        bonus *= 2;
                    else if (abs(eval) > 0)
                        bonus *= 1;
    ...
    if (   !PvNode
        ...
        &&  eval - futility_margin(depth, improving, us == thisThread->rootColor ? thisThread->cachedAggressiveness : 100) * (1 + pos.check_counting() + 2 * pos.must_capture() + pos.extinction_single_piece() + !pos.checking_permitted()) >= beta
    ...
    probCutBeta = beta + (209 + 20 * !!pos.flag_region(~pos.side_to_move()) + 50 * pos.captures_to_hand()) * (1 + pos.check_counting() + pos.extinction_single_piece()) - 44 * improving + (us == thisThread->rootColor ? (thisThread->cachedAggressiveness - 100) / 5 : 0);
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          &&  tte->depth() >= depth - 3)
      {
          ...
          else if (ttValue >= beta)
          {
              ...
              if (value >= beta)
               ...
               && depth > 6 - (us == thisThread->rootColor ? (thisThread->cachedAggressiveness - 100) / 25 : 0)
      ...
      // Track material difference for sacrifice detection (Patricia-style)
      {
          int matDiff = pos.non_pawn_material(WHITE) - pos.non_pawn_material(BLACK);
          (ss+1)->materialDiff = matDiff;
      }
          ...
          && (!PvNode || ss->ply > 1 || thisThread->id() % 4 != 3))
      {
          ...
          // Aggressiveness: reduce LMR for attacking moves
          int aggressiveness = (us == thisThread->rootColor) ? thisThread->cachedAggressiveness : 100;
          if (aggressiveness > 100 && (givesCheck || captureOrPromotion))
              r -= (aggressiveness - 100) / 50;
