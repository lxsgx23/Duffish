...
namespace {
  ...
  // Futility margin
  Value futility_margin(Depth d, bool improving, bool isRootPlayer) {
    int aggressiveness = isRootPlayer ? int(Options["Aggressiveness"]) : 100;
  ...
  Value value_draw(Thread* thisThread, Color us) {
    Value contempt = (us == thisThread->rootColor) ? Value(int(Options["Contempt"])) : VALUE_ZERO;
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              int dynamicComp = int(Options["DynamicComp"]);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !PvNode
        ...
        &&  eval - futility_margin(depth, improving, us == thisThread->rootColor) * (1 + pos.check_counting() + 2 * pos.must_capture() + pos.extinction_single_piece() + !pos.checking_permitted()) >= beta
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        int aggressiveness = (us == thisThread->rootColor) ? int(Options["Aggressiveness"]) : 100;
    ...
    probCutBeta = beta + (209 + 20 * !!pos.flag_region(~pos.side_to_move()) + 50 * pos.captures_to_hand()) * (1 + pos.check_counting() + pos.extinction_single_piece()) - 44 * improving + (us == thisThread->rootColor ? (int(Options["Aggressiveness"]) - 100) / 5 : 0);
          ...
          &&  tte->depth() >= depth - 3)
      {
          ...
          else if (ttValue >= beta)
          {
              ...
              if (value >= beta)
               ...
               && depth > 6 - (us == thisThread->rootColor ? (int(Options["Aggressiveness"]) - 100) / 25 : 0)
