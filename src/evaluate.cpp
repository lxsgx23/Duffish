...
#undef S
  ...
  Value Evaluation<T>::value() {
    ...
    score += me->imbalance() + (pos.side_to_move() == pos.this_thread()->rootColor ? pos.this_thread()->trend : SCORE_ZERO);
    ...
    if (lazy_skip(LazyThreshold1) && pos.this_thread()->cachedIsChessVariant)
        goto make_v;
    ...
    if (lazy_skip(LazyThreshold2) && pos.this_thread()->cachedIsChessVariant)
        goto make_v;
...
Value Eval::evaluate(const Position& pos) {
  ...
  // Patricia-style eval decay: additional aggressive decay based on halfmove clock
  // to incentivize active play and discourage shuffling even without n_move_rule
  int evalDecay = pos.this_thread()->cachedEvalDecay;
  ...
  // SacBonus: reward positions where the side to move has sacrificed material
  // Asymmetric: only applied for root player
  int sacBonus = (pos.side_to_move() == pos.this_thread()->rootColor) ? pos.this_thread()->cachedSacBonus : 0;
  if (sacBonus > 0)
  {
      int matDiff = pos.non_pawn_material(WHITE) - pos.non_pawn_material(BLACK);
      int matImbalance = pos.side_to_move() == WHITE ? -matDiff : matDiff;
      if (matImbalance > 0)
          v += Value(sacBonus * matImbalance / 200);
  }

  // Material scaling (Patricia-style): when winning, scale eval by material
  // to discourage trading down and encourage continuing the attack.
  // More material on board = higher scaled eval when winning.
  int matScale = (pos.side_to_move() == pos.this_thread()->rootColor) ? pos.this_thread()->cachedMatScale : 0;
  if (matScale > 0 && v != VALUE_ZERO)
  {
      int totalMat = pos.non_pawn_material(WHITE) + pos.non_pawn_material(BLACK);
      int multiplier = 1024 + matScale * totalMat / 3200;
      bool ourSide = (pos.side_to_move() == pos.this_thread()->rootColor);
      bool isWinning = (v > 0 && ourSide) || (v < 0 && !ourSide);
      if (isWinning)
      {
          Value scaledV = Value(int(v) * multiplier / 1024);
          v = scaledV;
      }
  }

  // Aggressiveness: amplify evaluation for the side with more attacking potential
  // Asymmetric: only applied for root player
  int aggressiveness = (pos.side_to_move() == pos.this_thread()->rootColor) ? pos.this_thread()->cachedAggressiveness : 100;
  if (aggressiveness != 100)
  {
      int aggressFactor = (aggressiveness - 100);
      v += Value(aggressFactor * int(v) / 400);
  }

  // Defense-breaking bonus for root player (Xiangqi-specific)
  // Rewards the root player for capturing opponent's advisors (FERS) and elephants (ELEPHANT)
  // Asymmetric: only applied for root player
  if (pos.side_to_move() == pos.this_thread()->rootColor)
  {
      Color them = ~pos.side_to_move();
      int advisorBreakBonus = pos.this_thread()->cachedAdvisorBreakBonus;
      int bishopBreakBonus = pos.this_thread()->cachedBishopBreakBonus;

      if (advisorBreakBonus > 0 && (pos.piece_types() & FERS))
      {
          int missingAdvisors = std::max(0, 2 - pos.count<FERS>(them));
          v += Value(advisorBreakBonus * missingAdvisors);
      }

      if (bishopBreakBonus > 0 && (pos.piece_types() & ELEPHANT))
      {
          int missingElephants = std::max(0, 2 - pos.count<ELEPHANT>(them));
          v += Value(bishopBreakBonus * missingElephants);
      }
  }
