...
Value Eval::evaluate(const Position& pos) {
  ...
  int sacBonus = (pos.side_to_move() == pos.this_thread()->rootColor) ? int(Options["SacBonus"]) : 0;
  if (sacBonus > 0)
  {
  ...
  // Asymmetric: only applied for root player
  int aggressiveness = (pos.side_to_move() == pos.this_thread()->rootColor) ? int(Options["Aggressiveness"]) : 100;
